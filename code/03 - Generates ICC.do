...
local first_loop 1
foreach outcome in "total_output" "total_productivity" "total_output_winsor" "total_productivity_winsor" {
	foreach crop in `all_crops' {
		foreach winsorized_option in "yes" "no" {	 
			...
			preserve
				...
				if "`outcome'" == "total_output_winsor" {
					gen outcome = "Production"
					gen winsorized = "yes"
				}
				else if "`outcome'" == "total_productivity_winsor" {
					gen outcome = "Production per hectare"
					gen winsorized = "yes"
				}
				gen cropid = "`crop'"
...
merge 1:m outcome cropid winsorized using "icc_data_temp.dta"
drop _merge
